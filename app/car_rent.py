...
async def get_offers_from_sourses(sources: list[str]) -> list[dict]:
    ...
    res = await asyncio.gather(*[get_offers(source) for source in sources])
...
async def chain_combine_service_offers(
    ...
    # do some logic
    print('chain_combine_service_offers')
    request_sem = Semaphore(MAX_PARALLEL_AGG_REQUESTS_COUNT)
    tasks = [asyncio.create_task(service_worker(inbound, outbound, request_sem)) for _ in range(WORKERS_COUNT)]
...
async def chain_filter_offers(
    ...
    **kw,
    ...
    while True:
        ...
        data = ctx.data
        if brand:
            data = [i for i in data if i.get('brand') == brand]
        if price:
            data = [i for i in data if i.get('price') <= price]
        
        outbound.put_nowait(PipelineContext(ctx.user_id, data))
        await asyncio.sleep(0.1)
...
async def chain_book_car(
    ...
    """
    tasks = [asyncio.create_task(booking_worker(inbound, outbound)) for _ in range(1)]
...
def run_pipeline(inbound: Queue[PipelineContext], loop) -> Queue[PipelineContext]:
    ...
    loop.create_task(chain_combine_service_offers(inbound, outbound_combine_service))
    loop.create_task(chain_filter_offers(outbound_combine_service, outbound_filter))
    loop.create_task(chain_book_car(outbound_filter, outbound_book_car))
